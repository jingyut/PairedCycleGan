...
class DResnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=True, padding_type='reflect'):
        ...
        super(DResnetGenerator, self).__init__()
        ...
        model += [nn.Conv2d(ngf, ngf, kernel_size=3, padding=0),
                norm_layer(ngf),
                 nn.ReLU(True)]
...
class DResnetBlock(nn.Module):
    ...
    def build_conv_block(self, dim, padding_type, norm_layer, use_dropout, use_bias, dilation):
        """
        Construct a dilated convolutional block.
        Parameters:
            ...
            dilation            -- the dilation of the conv block 
        Returns a dilated conv block (with a conv layer, a normalization layer, and a non-linearity layer (ReLU))
        """
        conv_block = []
        ...
        conv_block += [nn.Conv2d(dim, dim, kernel_size=3, padding=dilation-1, bias=use_bias, dilation=dilation), norm_layer(dim), nn.ReLU(True)]
        if use_dropout:
        ...
        if padding_type == 'reflect':
            conv_block += [nn.ReflectionPad2d(1)]
        conv_block += [nn.Conv2d(dim, dim, kernel_size=3, padding=dilation-1, bias=use_bias,dilation=dilation), norm_layer(dim)]
